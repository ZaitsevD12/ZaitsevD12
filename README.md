# Привет, я Данила Зайцев 👋

![Иллюстрация профиля]([https://img.undraw.co/programming.svg](https://private-user-images.githubusercontent.com/74038190/241765440-80728820-e06b-4f96-9c9e-9df46f0cc0a5.gif?jwt=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..ELd7SA9uUTHZmNDAZTCOiIv0EY3MdJkT3acAZdQlfGM))

👨‍💻 **Обо мне** ✨ Junior Java Developer | Ex-Kommo Intern | КИПФИН '23 (WorldSkills) | Финалист Cup IT 2025 (аналитика данных)

## 🛠 Tech Stack

### Языки и инструменты:
![Java](https://img.shields.io/badge/Java-21-orange?style=flat&logo=java) ![JavaScript](https://img.shields.io/badge/JavaScript-yellow?style=flat&logo=javascript) ![Spring Boot](https://img.shields.io/badge/Spring%20Boot-3.3-green?style=flat&logo=spring) ![React](https://img.shields.io/badge/React-18-blue?style=flat&logo=react) ![PostgreSQL](https://img.shields.io/badge/PostgreSQL-16-blue?style=flat&logo=postgresql) ![PHP](https://img.shields.io/badge/PHP-Zend/Laravel-purple?style=flat&logo=php) ![Docker](https://img.shields.io/badge/Docker-blue?style=flat&logo=docker) ![Python](https://img.shields.io/badge/Python-ML-yellow?style=flat&logo=python)

### DevOps:
![Linux](https://img.shields.io/badge/Linux-black?style=flat&logo=linux) ![Git](https://img.shields.io/badge/Git-orange?style=flat&logo=git)

## 📈 Статистика

![Статистика](https://github-readme-stats.vercel.app/api?username=ivan-ivanov-java&show_icons=true&theme=radical&hide_border=true)  
![Топ языки](https://github-readme-stats.vercel.app/api/top-langs/?username=ivan-ivanov-java&layout=compact&theme=radical&hide_border=true)

## 📚 Проекты

✨ **[MyPeak](https://github.com/ZaitsevD12/Project-Comitia.git)**: Telegram Mini-App (Steam API, верификация, weighted ratings, React UI). VPS-deploy.  
✨ **[Диплом](https://github.com/ZaitsevD12/Fitness-Consultant.git)**: Full-stack JS (Express/React, API integration).  
✨ **[ML Project](https://github.com/ZaitsevD12/RiskPredict.git)**: Python ML для соревнований (scikit-learn, data analysis).  

Контакты: zaitseffdb@gmail.com | Telegram: @Sonybvc
